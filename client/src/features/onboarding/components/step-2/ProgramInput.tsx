...
import { educationPrograms } from '../../schemas/onboardingStepTwoSchema';
...
export const ProgramInput = React.forwardRef<HTMLDivElement, ProgramInputProps>(
  ({ value, onChange, id }, ref) => {
    ...
    const filteredOptions = React.useMemo(() => {
      if (!search) return educationPrograms;
      const searchLower = search.toLowerCase();
      return educationPrograms.filter((option) =>
        option.label.toLowerCase().includes(searchLower)
      );
    }, [search]);

    const selectedLabel = React.useMemo(() => {
      const selected = educationPrograms.find((option) => option.value === value);
